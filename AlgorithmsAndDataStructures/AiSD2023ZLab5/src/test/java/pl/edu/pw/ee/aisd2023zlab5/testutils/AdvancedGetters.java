...
public final class AdvancedGetters {

    private AdvancedGetters() {
    }

    public static List<?> getHeapElements(HeapMin<?> heap) {
        String fieldName = "elements";

        List<?> elements = null;
        try {
            Field field = heap.getClass().getDeclaredField(fieldName);
            field.setAccessible(true);
            elements = (List<?>) field.get(heap);
